/*
 * Communicating with Arduino - UART Interrupt Mode
...
/* Enable USART2_IRQn in CubeMX for the interrupt version. */
static volatile uint8_t rx, ready;
void USART2_IRQHandler(void){HAL_UART_IRQHandler(&huart2);}
void HAL_UART_RxCpltCallback(UART_HandleTypeDef *h)
{
    if(h->Instance==USART2){ready=1; HAL_UART_Receive_IT(&huart2,(uint8_t*)&rx,1);}
}
int main(void)
{
    ...
    while(1){if(ready){ready=0; HAL_UART_Transmit(&huart2,(uint8_t*)&rx,1,HAL_MAX_DELAY);}else __WFI();}
}
