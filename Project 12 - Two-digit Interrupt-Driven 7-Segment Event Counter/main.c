/*
 * Two-digit Interrupt-Driven 7-Segment Event Counter
...
static const uint16_t digit_pins[2]={GPIO_PIN_7,GPIO_PIN_8};
...
{
    if(p==GPIO_PIN_13) count=(count+1U)%100U;
}
static void Button_Init(void)
{
    GPIO_InitTypeDef g={0};
    __HAL_RCC_GPIOC_CLK_ENABLE(); __HAL_RCC_SYSCFG_CLK_ENABLE();
    g.Pin=GPIO_PIN_13; g.Mode=GPIO_MODE_IT_FALLING; g.Pull=GPIO_NOPULL;
    HAL_GPIO_Init(GPIOC,&g);
    HAL_NVIC_SetPriority(EXTI15_10_IRQn,2,0); HAL_NVIC_EnableIRQ(EXTI15_10_IRQn);
}
int main(void)
{
    HAL_Init();SystemClock_Config();Display_Init();Button_Init();
