...
void HAL_GPIO_EXTI_Callback(uint16_t p){if(p==GPIO_PIN_13)HAL_GPIO_TogglePin(GPIOA,GPIO_PIN_5);}
static void IO_Init(void)
{
    GPIO_InitTypeDef g={0};__HAL_RCC_GPIOA_CLK_ENABLE();__HAL_RCC_GPIOC_CLK_ENABLE();__HAL_RCC_SYSCFG_CLK_ENABLE();
...
int main(void)
{
    HAL_Init();SystemClock_Config();IO_Init();__HAL_RCC_PWR_CLK_ENABLE();
    while(1){HAL_PWR_EnterSTOPMode(PWR_LOWPOWERREGULATOR_ON,PWR_STOPENTRY_WFI); SystemClock_Config();}
}
