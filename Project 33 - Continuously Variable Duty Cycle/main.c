/*
 * Continuously Variable Duty Cycle
...
int main(void)
{
    HAL_Init(); SystemClock_Config(); PWM_Init(999,100); int32_t duty=100,step=10;
    while (1) { __HAL_TIM_SET_COMPARE(&htim2,TIM_CHANNEL_2,(uint32_t)duty);
        duty+=step; if(duty>=950||duty<=50) step=-step; HAL_Delay(20); }
}
