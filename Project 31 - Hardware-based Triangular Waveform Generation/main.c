/*
 * Hardware-based Triangular Waveform Generation
...
int main(void)
{
    ...
    while (1) {
        for(uint32_t v=0;v<4095;v+=128) HAL_DAC_SetValue(&hdac,DAC_CHANNEL_1,DAC_ALIGN_12B_R,v);
        for(int32_t v=4095;v>=0;v-=128) HAL_DAC_SetValue(&hdac,DAC_CHANNEL_1,DAC_ALIGN_12B_R,(uint32_t)v);
    }
}
